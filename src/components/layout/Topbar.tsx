...
export const Topbar: React.FC = () => {
  ...
  return (
    <div className="flex items-center justify-end px-6 pt-5 pb-1 z-40 shrink-0">
      {/* Notification Bell Dropdown Container */}
      <div className="relative">
        <button
          onClick={() => setShowNotifications(!showNotifications)}
          className="relative p-2 rounded-xl bg-white border border-slate-200 text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-all shadow-xs cursor-pointer"
          aria-label="Notifications"
        >
          <Bell className="w-4 h-4" />
          {unreadCount > 0 && (
            <span className="absolute -top-1 -right-1 w-4 h-4 bg-indigo-600 text-white font-bold text-[10px] rounded-full flex items-center justify-center border-2 border-slate-50">
              {unreadCount}
            </span>
          )}
        </button>
              ...
              {notifications.length === 0 ? (
                ...
                notifications.map((n) => (
                  <div
                    ...
                    className={`p-3.5 hover:bg-slate-50 cursor-pointer transition-colors flex items-start space-x-3 ${!n.read ? 'bg-indigo-50/40' : ''
                      }`}
                  >
